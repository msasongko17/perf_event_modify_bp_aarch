...
static inline int create_watchpoint2(uintptr_t address, int type, int len) {
    // Perf event settings
    struct perf_event_attr pe = {
        .type                   = PERF_TYPE_BREAKPOINT,
        .size                   = sizeof(struct perf_event_attr),
...
int main(){
        ...
        clock_t t3  = clock();
        int fd = create_watchpoint2((uintptr_t) &dummy[0], HW_BREAKPOINT_W | HW_BREAKPOINT_R, HW_BREAKPOINT_LEN_1);
        for(int i = 1 ; i <= N; i++){
                if (i & 1)
